...
export const APP_BUILD = 170; // Увеличивается при каждой сборке

// Ключи для хранения
const VERSION_STORAGE_KEY = "rp_app_version";
const BUILD_STORAGE_KEY = "rp_app_build";
const LAST_MIGRATION_KEY = "rp_last_migration";
...
class VersionManager {
  ...
  async initialize(): Promise<MigrationResult> {
    // Hard timeout — never block app startup for more than 3 seconds
    return Promise.race([
      this._doInitialize(),
      new Promise<MigrationResult>((resolve) =>
        setTimeout(() => {
          logger.warn("[VersionManager] Initialization timed out (3s), skipping");
          resolve({ success: true, migrationsRun: [], errors: ["timeout"] });
        }, 3000),
      ),
    ]);
  }

  private async _doInitialize(): Promise<MigrationResult> {
  ...
  private async validateSupabaseSession(): Promise<void> {
    ...
    try {
      ...
      const lastMigration = localStorage.getItem("rp_last_migration");
      if (!lastMigration) {
  ...
  async clearAllCaches(): Promise<void> {
    ...
    const keysToKeep = ["rp-auth-token"];
