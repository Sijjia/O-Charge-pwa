...
const navItems: NavItem[] = [
  { to: "/corporate/dashboard", icon: "solar:widget-2-bold-duotone", label: "Обзор" },
  { to: "/corporate/employees", icon: "solar:users-group-two-rounded-bold-duotone", label: "Сотрудники" },
  { to: "/corporate/reports", icon: "solar:chart-square-bold-duotone", label: "Отчёты" },
  { to: "/corporate/invoices", icon: "solar:document-text-bold-duotone", label: "Счета" },
];
...
export function CorporateLayout() {
  ...
  const { data: profileData } = useQuery({
    ...
    queryFn: () => fetchJson("/api/v1/corporate/profile", { method: "GET" }, ProfileResponseSchema),
    retry: false,
    staleTime: 5 * 60_000,
  });

  const companyName = useMemo(() => {
    return profileData?.company_name || profileData?.name || "Компания";
  }, [profileData]);
    ...
    <>
      {navItems.map((item) => (
        <NavLink
          key={item.to}
          to={item.to}
          end={item.to === "/corporate/dashboard"}
          onClick={onItemClick}
          className={({ isActive }) =>
          ...
          <div className="flex items-center gap-2.5 flex-1">
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-400 dark:text-zinc-500 font-medium uppercase tracking-wider">
                Corporate <span className="font-mono text-zinc-300 dark:text-zinc-600">v{APP_VERSION}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-3 border-t border-zinc-200 dark:border-zinc-800 space-y-3">
          ...
          <div className="flex items-center gap-3 px-3 py-2">
            <div className="w-8 h-8 bg-zinc-100 dark:bg-zinc-800 rounded-full flex items-center justify-center">
              <Icon icon="solar:buildings-bold-duotone" width={16} className="text-zinc-500 dark:text-zinc-400" />
            </div>
        ...
        <div className="h-full flex items-center justify-between px-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-400 dark:text-zinc-500 -mt-0.5">Corporate <span className="font-mono text-zinc-300 dark:text-zinc-600">v{APP_VERSION}</span></p>
            </div>
          </div>
              ...
              <div className="p-3 space-y-3">
                ...
                <div className="flex items-center gap-3 px-3 py-3 bg-zinc-50 dark:bg-white/5 rounded-xl">
                  <div className="w-10 h-10 bg-zinc-100 dark:bg-zinc-800 rounded-full flex items-center justify-center">
                    <Icon icon="solar:buildings-bold-duotone" width={20} className="text-zinc-500 dark:text-zinc-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-zinc-900 dark:text-white truncate">
                      {companyName}
                    </p>
                    <p className="text-xs text-zinc-400 dark:text-zinc-500">Корпоративный</p>
                  </div>
                </div>
