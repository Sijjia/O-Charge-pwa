...
describe("useCharging hooks", () => {
  ...
  describe("useStartCharging", () => {
    it("должен успешно начать зарядку", async () => {
      ...
      vi.mocked(rpApi.getStationStatus as any).mockResolvedValue({
        available_for_charging: true,
      });
      vi.mocked(rpApi.startCharging as any).mockResolvedValue({
        success: true,
    ...
    it("должен обработать ошибку при недоступной станции", async () => {
      ...
      vi.mocked(rpApi.startCharging as any).mockRejectedValue(
  ...
  describe("useStopCharging", () => {
    it("должен успешно остановить зарядку", async () => {
      const { rpApi } = await import("@/services/rpApi");
      vi.mocked(rpApi.stopCharging as any).mockResolvedValue({
        success: true,
        message: "ok",
      });
  ...
  describe("useChargingStatus", () => {
    it("должен получить статус зарядки", async () => {
      const { rpApi } = await import("@/services/rpApi");
      vi.mocked(rpApi.getChargingStatus as any).mockResolvedValue({
        success: true,
        session: {
          id: "s1",
          status: "started",
